...
@section('content')
    @include('admin_dashboard.page_name')
                ...
                <div class="card-body">
                	<form class="needs-validation" novalidate method="POST" action="{!! url('/save_parent_menu'); !!}">
                		{{ csrf_field() }}

						<div class="form-group row {{ $errors->has('menu_name') ? ' has-error' : '' }}">
							<label for="menu_name" class="col-sm-2 col-form-label">Menu name</label>
							<div class="col-sm-10">
								<input type="text" class="form-control {{ $errors->has('menu_name') ? 'is-invalid' : '' }}" id="menu_name" name="menu_name" placeholder="menu name" value="{{ old('menu_name')}}" required>
